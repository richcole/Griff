...
(* $Id: cgi.ml,v 1.1.1.1 2004/12/31 01:12:42 rj-cole Exp $ *)

(* Code from wserver.ml, (C) 1997 Daniel de Rauglaudre, INRIA. *)

(* Edited a bit by Richard Cole to fit my needs *)
...
let args = 
  Rb_set.map_from_list (parse_args ())

let arg ~name ~default = 
  Rb_set.some_default default (Rb_set.image name args)

let arg_no_default ~name =
  try
    Rb_set.some (Rb_set.image name args)
  with
    | Rb_set.NotFound -> 
	Printf.fprintf stderr "Missing argument %s\n" name;
	raise Not_found
;;
