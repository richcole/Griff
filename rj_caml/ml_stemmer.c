...
/* RJ: I grabber the following code from a website and have hacked it
   to provide an ocaml interface.
...
value ml_stem(value s)
{
  ...
  strncpy(str_buf, String_val(s), sizeof(str_buf)-1);
  str_buf[sizeof(str_buf)-1] = 0;

  length = strlen(str_buf);
  length = stem(str_buf,0,length-1)+1;

  if (length < strlen(str_buf)) 
    str_buf[length]=0; 
  else 
    str_buf[sizeof(str_buf)-1] = 0;

  return_value( copy_string(str_buf) );
};
